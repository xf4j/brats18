...
class ResUNet3D(BaseModel):
    def __init__(self, sess, checkpoint_dir, log_dir, training_subjects, testing_subjects, testing_during_training, 
        ...
        self.use_n4 = model_config['use_n4'] == 'True'
        self.use_denoise = model_config['use_denoise'] == 'True'
        self.use_conv_for_pooling = model_config['use_conv_for_pooling'] == 'True'
        self.deploy_nstride = int(model_config['deploy_nstride'])
        
        # Check n4 and denoise
        if self.use_n4 == False and self.use_denoise == True:
            raise Exception("Do not support denoise without n4 bias correction")
        if self.use_n4 == True:
            if self.use_denoise:
                self.affix = '_corrected_denoised.nii.gz'
            else:
                self.affix = '_corrected.nii.gz'
        else:
            self.affix = '.nii.gz'
